...
const { update } = require("../models/skills/domains.model.js");


const createDomains = async (domainData)=>{

    const domainInDB = await domain.findOne({domainName:{'$regex': domainData.domainName,$options:'i'}});
...
const readDomainData = async (domainReq) =>{
    ...
    const domainInDB = await domain.findOne({domainName:{'$regex':domainReq,$options:'i'}});
